...
rutasUsuario.post('/crear', (request: Request, response: Response) => 
{
    ...
    var caracteres3 = /(^[A-Za-zÁÉÍÓÚáéíóúñÑ ]{3,150})+$/g;
    if(caracteres3.test(request.body.direccion) == false){

        return response.json({
            ok: false,
            mensaje: 'Debe ingresar una  direccion valida, entre 3 y 150 caracteres'
        });
    }

    request.body.comunidad = '64a0d54909003038cecc7443';
...
rutasUsuario.get('/esta-el-vecino',(request:any,response: Response) =>{
    response.send("Si, el vecino se encuentra en casa.")
})
