...
const emisor = await Emisor.find({estado: estadoEmitir, comunidad:request.usuario.comunidad})
...
rutasEmisor.post('/solicitud', [verificaToken], async (req: any, res: Response) => {
         ...
         try {
             ...
             const rutSolicitud = format(req.body.rut);
             ...
             const rutFormateado = format(req.body.rut);
...
const emisor = await Emisor.find({estado: estadoEmitir, usuario:request.usuario._id})
                        .sort({_id: -1}) // se ordena desde el mas nuevo
                        .skip(skip)
                        .limit(10) // se muestra 10 registros por pagina
                        .populate({path:'usuario',select: '-password'}) // se llena la tabla con los datos del usuario, excepto la contraseña
