...
import { Publicacion, OpcionesCategoria, OpcionesEstado } from '../modelos/publicacionBDModel';
...
cron.schedule('0 0 * * *', EstadoPublicacion);

//Renovar publicacion despues de 2 semanas de la creacion de esta
rutasPublicacion.post('/renovar/:publicacionId', [verificaToken], async (req: any, res: Response) =>{
    try{
...
//Crear publicacion
rutasPublicacion.post('/crear', [verificaToken], async (req:any, res: Response) =>{
...
rutasPublicacion.delete('/borrarPublicaciones/:publicacionId', [verificaToken], async(req: any, res:Response)=>{
    ...
    try {
        ...
        const usuario = await Usuario.findOne({ _id: usuarioId, rol: 1});
        if(!usuario){
