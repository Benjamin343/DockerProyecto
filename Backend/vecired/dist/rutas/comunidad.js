...
rutasComunidad.get('/puedeEmitir', [autenticacion_1.verificaToken], (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const comunidadId = request.usuario.comunidad;
        const comunidad = yield comunidadBDModel_1.Comunidad.findOne({ _id: comunidadId }).exec();
        if (!comunidad) {
        ...
        const puedeEmitir = comunidad.emitirCertificado;
        response.json({
            puedeEmitir,
        });
    }
        ...
        response.status(500).json({
            ok: false,
            mensaje: 'Error al obtener el atributo puedeEmitir',
            error,
        });
    }
}));
